...
**Phase 8 — Spend (on/off-ramp and everyday payments):** close the loop from "trade and earn" to "spend," turning stablecoin balances into everyday money rather than a destination that only ever gets bridged or withdrawn. Licensed fiat on/off-ramp partners let users fund a balance from and cash out to local currency; a stablecoin spend card (or QR/point-of-sale rail) lets that balance pay for real-world expenses. The differentiator versus a generic crypto debit card: every non-native-currency swipe routes its FX conversion through Keystone's own Book instead of a card network's markup, and that captured spread flows back to the Reserve as real yield — spend and earn on the same rails. Sequenced deliberately last: it depends on Phase 3's regional pairs (BRLA/JPYC/MXNB/PHPC/etc. — a card is only as good as the local-currency pairs it can convert into) and carries real regulatory weight (KYC, e-money licensing, card issuing partnerships, jurisdiction-by-jurisdiction compliance) that has nothing to do with what's being judged at the hackathon. Vision item for the roadmap slide, not a build target now.
**Phase 9 — Remove the single-operator push from price data:** as of 2026-08-10, `reference-feed` reads a real, verified Pyth price feed live on Arc Testnet (`ARC_TESTNET_CONTRACTS.PYTH`, see `DECISIONS.md`) but still *pushes* the result into `MockOracle` on a timer — bots, `reserve-keeper`, and `KeystoneReserve`'s NAV math all read `MockOracle`, not Pyth directly. Closing that gap means two things: (a) bots and `reserve-keeper` read Pyth on-chain directly for their quoting mid instead of `MockOracle` — cheap, no redeploy; (b) `KeystoneReserve`'s NAV valuation of idle EURC reads Pyth too, which is the harder half since its `ORACLE` reference is `immutable` — needs a small IPyth-adapter contract and a `KeystoneReserve` redeploy, migrating whatever live TVL exists at the time. Not needed for the hackathon submission; real next step once the push-based version has proven itself.
